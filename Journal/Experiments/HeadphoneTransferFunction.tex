...
\begin{itemize}
	\item 93.6 dB due to being a pressure field. 1 $k$Hz is used as calibration signal. \todoOliver{again calibration??}
...
\begin{itemize}
	...
	\item The measuring amplifier:
		\begin{itemize}
			\item "Input Section Gain" is set to +20 dB in proportion to calibration. \todoOliver{so +10dB?? Write that }
			\item 22.4 $k$Hz low-pass filter is enabled.
			\item 22.4 $k$Hz high-pass filter is enabled. \todoOliver{And very little sound is let through????}
		\end{itemize}
...
\begin{enumerate}
	\item Start Simulink \textsuperscript{\textregistered} and run file  \path{SimulinkSystemHeadPhone.xls}.
...
\begin{enumerate}
	...
	\item Open MATLAB\textsuperscript{\textregistered} and run script \path{HeadPhoneScript.m}.
\end{enumerate}
...
These files are the results of performing the six second simulation which gives a number of 288,768 samples, for each generated .wav-file. \todoOliver{Again - not 6*48000??}
...
\end{figure}
\todoOliver{Here they are not averaged - should be the same as in CP}
...
\begin{itemize}
	\item Placement of the actuators and test-devices. \todoOliver{again do we just mean headphone?}	
	...
	\item Movement of instruments. \todoOliver{Did they move? Who moved them?? If they didn't move don't write this}
