...
\begin{itemize}
	...
	\item Error micro calibration signal yields a spike at 61 dB SPL @1000 Hz, the expected value is 95,6 dB which then requires a +34.6 dB SPL adjustment.
	\item Reference micro calibration signal yields a spike at 66.07 dB SPL @1000 Hz, the expected value is 94 dB which then requires a +28 dB SPL adjustment.
\end{itemize}
...
\begin{enumerate}
	...
	\begin{itemize} 
		...
		\item The simulation runs for 7 seconds. \todoMax{I think we used 7 - pls check}
	\end{itemize}
...
In order to have an idea of the impulse response,  \autoref{DT770CancellationPathImpulseResponseCrop} display the first 200 samples of the impulse response.
