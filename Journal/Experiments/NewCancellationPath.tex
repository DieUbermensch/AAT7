...
\end{figure}
\todoMax{The amplifiers are both in the anechoic }
...
\begin{itemize}
	...
	\item Error micro calibration signal yields a spike at 61 dB SPL @1000 Hz, which then requires a +34.5 dB SPL adjustment.
	\item Reference micro calibration signal yields a spike at 66.07 dB SPL @1000 Hz, which then requires a +28 dB SPL adjustment.
\end{itemize}
\todoMax{write what they should have been}
...
\begin{enumerate}
	...
	\item Control and calibrate item 2 using item 9.
	\item Control and calibrate item 4 using item 5.
	\item Place item 3 onto item 1 ("Henry"'s head and ears).
	\item Insert item 2's connector into input of item 8.
	\item Connect item 8 to item 4 and connect item 6 to item 7 via USB.
	\item Insert item 3's connector into input1/L of item 6.
	\item Insert item 5's connector into input1/R of item 6.
\end{enumerate}
\todoMax{Sry we have moved away from this way of writing it with Item, as it's confusing as hell. Would you mind changing it to match the corrected CP trans?}
...
\begin{enumerate}
	...
	\begin{itemize} 
		...
		\item The simulation runs for seconds. \todoMax{I think we used 7 - pls check}
	\end{itemize}
...
These files are the results of performing the six \todoMax{check - also the amount of samples of course} second simulation which gives a number of 288,768 samples, for each generated .wav-file.
...
\todoMax{Here we use two prepolarized mics?? So should se no 180 phase shift. The answer is that the spmMic does not invert phase (I guess)}
...
Taking the first 256 coefficients of the impulse response gives the following cropped impulse response, as seen on \autoref{DT770CancellationPathImpulseResponseCrop}.
\todoMax{Should be 960 coef - the 256 is a very old wrong number}
