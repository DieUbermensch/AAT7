...
\begin{itemize}
	\item 93.6 dB due to being a pressure field microphone $@$ 250 Hz is used as calibration signal. \todoOliver{We have later found 95.6dB using an other calibrator - are you certain about this value???}
...
\begin{itemize}
	...
	\item Roland Edirol:
		\begin{itemize}
			...
			\item "PHONES" is set to maximum volume. \todoOliver{Should we mention, that this might be a bad idea?-meybe listen if recordings are distorted??}\
		\end{itemize}		
...
These files are the results of performing the six second simulation which gives a number of 288,768 samples, \todoOliver{This is not 48000 *6 ??} for each generated .wav-file.
...
\begin{itemize}
	\item Placement of the actuators and test-devices. (Position of the headset on the ear  influence the frequency response at high frequency which is not the main scope of study). \todoOliver{Do we just mean the headphones? Because then write headphones}
