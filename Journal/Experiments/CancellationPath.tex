...
Firstly the cross correlation between Mic[i] and Ref[i] is found. This is done by cross correlating the two signals signals. Secondly the signal is taken into the frequency domain by applying the FT to the signal. This is expressed in \autoref{FrequencyResponseEq2}.
...
\end{figure}
A microphone with external polarization was used, it implies that positive pressures mesured leads to negative voltage \cite{michandbook}. It affects the phase response by 180 degrees and inverts the sign of the impulse response. 
...
From \autoref{CancellationPathImpulseResponseCompare}, 250 coefficients is shown ot be sufficient for representing the impulse response with good accuracy. However 256 coefficients is deemed being more practical when it comes to binary operations, this value is chosen. 
