...
\begin{enumerate}
	\item Open Simulink\textsuperscript{\textregistered} and open and run file \path{SimulinkSystemCancellationPath.xls}
	\begin{itemize} 
...
\begin{enumerate}
	...
	\item Open MATLAB\textsuperscript{\textregistered} and run script \path{CancellationPathScript.m}
\end{enumerate}
...
\end{figure}
A microphone with external polarization was used, it implies that positive pressures mesured leads to negative voltage \autoref{michandbook}. It affects the phase response by 180 degrees and inverse the sign of the impulse response. 
...
From \autoref{CancellationPathImpulseResponseCompare}, 250 coefficients are sufficient to represent the impulse response with accuracy.  256 coefficients being more easy when it comes to binary operations, this value will be chosen. 

\subsection{Error Sources / Tolerances}
\begin{itemize}
	\item Placement of the actuators and test-devices. (Position of the headset on the ear  influence the frequency response at high frequency which is not our main scope of study)
