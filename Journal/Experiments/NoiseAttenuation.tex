...
%	\input{figures/AttenuationExperiment/AttenuationTestSetup.tex}
	\includegraphics[width=0.4\textwidth]{AttenuationExperiment/AttenuationTestSetup}
	\caption{Setup of Noise attenuation experiment.}
	\label{Fig:NoiseAttenuationExperimet}
\end{figure}
...
\begin{enumerate}
	...
	\item Run Simulink \path{AttenuationTest.slx} with testnumber equal to 1 and gain slider at 0 dB.
...
The test was conducted with 12 participants subject to 6 listening tests. The first test (enumerated 0) is omitted in analysis. The next three tests (1-3) were realized with a high sound level in the headset whereas the two last tests (4-5) were conducted with a lower sound level in the headset. 
...
\item All participants were from the institute of electronics at AAU. Quite a few studying acoustics and some having preliminary knowledge of the scope of the main project. Only one female participated in the test. The participants were not chosen at random.  
...
A given participant's results over different tests are very stable, within $\pm$ 2.5 dB which was the calibration uncertainty. 
