...
\begin{table}[h]
	...
	\begin{tabular}{ c c c } \toprule
		...
		5		&  Head And Torse Simulator "Valdemar" with SPL-meter					& 2150-04	\\
...
\begin{table} [h]
	...
	\begin{tabular}{c c} \toprule
		...
		Table							& 90				\\ \bottomrule 
	\end{tabular}
	\caption{Height of setup. Speakers are placed with tweeters in the bottom.}
...
\begin{figure}[H]
	\centering
%	\tikzsetnextfilename{AttenuationTestSetup}
%	\input{figures/AttenuationExperiment/AttenuationTestSetup.tex}
	\includegraphics[width=0.8\textwidth]{AttenuationExperiment/AttenuationTestSetup}
	\caption{Setup of Noise attenuation experiment.}
	\label{Fig:NoiseAttenuationExperimet}
\end{figure}
...
The Sound pressure levels of the sources are calibrated at the position of the listener. The calibration is made with all gain settings on the computer and in Simulink\textsuperscript{\textregistered} set to 0 dBFS (including the slider). "Valdemar" is placed at the listening position and the built in SPL-meter is used to find the levels of the noise from the two sets of speakers individually and combined. 
The same method is used to find the SPL of the sound in the headphones. The headphones are placed over the ears of "Valdemar" and a readout is made. The levels can be seen in \autoref{SPLtableNA}.
...
%	\input{figures/AttenuationExperiment/SchematicSetup.tex}
	\includegraphics[width=\textwidth]{AttenuationExperiment/AttenuationSetup}
...
\begin{enumerate}
	...
	\item Run Simulink \path{AttenuationTest.slx} with testnumber equal to 1 and gain slider at 0 dBFS.
...
%  Secondly, even if test results between each participants varies a lot, three population families can be find: "ultra sensitive" from - 26db to - 19db , "sensitive" from -14db to -8db  and "less sensitive" from -6db to 0db. 
...
%, our system needs to consider critical values of our experiment.


%Notes from CC ALL DONE
% more items on overview? 
% more than half of the participant's values are within 8-10 dB with extreme values going from 0 to -28.4.
% comment about less sensitive population in conclusion - new
% Comparison on stability of persons - that's not in the main! Only test stability is in the main OK
% boxplot OK
% comment on measurementlevels OK
% No comments on the diagrams OK
% histograms are inverted + -> - OK
% variance not used for anything - deleted
% New sorting into groups in conclusion OK
