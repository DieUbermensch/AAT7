...
\subsection{Procedure}
	...
	\begin{enumerate}
		\item Open Simulink and run file \path{OtherBrands.slx}.
		\begin{itemize} 
		...
		\end{itemize}
		\item Rename generated file \path{GeneratedANC} according to the headphone tested, and whether or not ANC was enabled. E.g. \path{Denonon[1].wav} for the first test of the Denon AH GC20 headhpone with ANC enabled. Names in \path{evaluatePerformance.m} is named accordingly.
...
\begin{enumerate}
	\item Open Matlab and run script \path{evaluatePerformance.m}.
\end{enumerate}
...
\begin{itemize}
	...
	\item \path{Sonyon[i].wav}			(Sony MDR-1000X  w/ ANC)
	\item \path{Sonyoff[i].wav}			(Sony MDR-1000X  w/o ANC)
	\item \path{Sennon[i].wav}	(Sennheiser PXC-550  w/ ANC)
	\item \path{Sennoff[i].wav}	(Sennheiser PXC-550  w/o ANC)
\end{itemize}
...
\begin{figure}[H]
	\centering
	%\tikzsetnextfilename{Sony1000Compare}
	%\input{../Journal/Experiments/TestofConsumerHeadphones/Plots/Sony1000Compare(fig).tex}
...
\begin{figure}[H]
	\centering
	%\tikzsetnextfilename{SennCompare}
	%\input{../Journal/Experiments/TestofConsumerHeadphones/Plots/SennCompare(fig).tex}
