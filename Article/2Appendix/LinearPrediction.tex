...
Levinson-Durbin is a method which can be used to find the estimated LPCs of a problem, recursively. \\
The algorithm takes the coefficients of the ACF in order to compute the RCs, the algorithm can be seen below: \\
...
\begin{equation}\label{LDInit}
	J_0=R[0]
\end{equation}
The algorithm works recursively, and the next step iterates for a total number of the framelength, $M$ in order to compute RCs for each $l$.
...
\begin{equation}\label{LDStep2}
	J_1 = J_{l-1} (1-k_l^2)
\end{equation}
Lastly gives the LPCs:
\begin{equation}
	a_i = a_i^{M}; \hspace{10mm} i = 1,2 \dotsc , M
\end{equation}
The Levinson-Durbin algorithm is characterized by being power efficient, as it does not directly invert a matrix, but rather estimates it.
