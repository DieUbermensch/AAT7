...
\\\\
If a feedforward LP FXLMS system is to be implemented in a product certain previous assumptions must be reconsidered. In the test setup shown in \autoref{fig:SystemOverview} the error microphone is placed inside the ear which is of course not practical in a product. This can be solved by the use of different techniques e.g. virtual sensing or virtual sound pressure as described in \cite{Hansen2}. The computational cost of the predictor is very high. With the chosen parameters, 15,000 instructions are needed per sample, as can be seen from \autoref
