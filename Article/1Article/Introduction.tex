...
\begin{figure}[H]
	...
	\caption{Denon AH-GC20, Bose QC25, Bose QC15 and B\&O H8 measured in anechoic chamber, averaged over five measurements where the headphones was refitted between measurements. Results are filtered using a 1/3 octave filter bank.}
	\label{fig:ANCcompare}
\end{figure}



The problem of a feedforward system, shown in figure \ref{fig:SystemOverview}, is the dependency of noise having a larger propagation delay from the noise measured (1), so the headphone loudspeaker (2) can produce a counter-phase signal in the desired point (3). 
\begin{figure}[H]
	\centering
...
\\\\
This paper presents and compares the feedforward FXLMS system and the feedforward LP FXLMS system using Wiener filtering. 
