...
Noise canceling headphones are used in various environments which have different noise characteristics. These characteristics vary from periodic low frequency noise (X-Y Hz), e.g. from machinery and plane turbines\cite{LowFrequency}, mid-range frequency noise (Y-Z Hz), e.g. speech\cite{MidFrequency}, to high frequency noise (Z-Q Hz), e.g. X\cite{HighFrequency}. The high frequency noise are naturally attenuated by using a closed headphone cup\cite{naturalAttenuation} and static low frequency noise are attenuated by the present consumer ANC headphones\cite{ConsumerANC}. Speech can be shown in figure \ref{fig:ANCcompare}, not to be attenuated by active noise cancellation in present market ANC headphones.  
