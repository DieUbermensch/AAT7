...
\vspace{-8mm}
\begin{description}
	\item[\text{$x_l[n]$}] $=x[n]x[n-l]$ and $x[n]$ is the input
	\item[\text{$w_l[n]$}] $=w[n]w[n+l]$
	\item[\text{$l$}] is the lag in the autocorrelation function
	\item[\text{$m$}] is the offset in the signal
	\item[\text{$N$}] is the frame size
\end{description}

The LPCs are determined in combination with the Yule-Walkers equation \autoref{eq:normal} to determine the LPCs.
