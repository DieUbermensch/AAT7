\large

\begin{minipage}{.43\columnwidth}
 \textbf{Optimizing the LP parameters}\\
The parameters; framelength $N$ and the overlap of frames $O$ are varied for a prediction length $P=10$ and $f_s=48$ $k$Hz. This is to determine the parameters for which the Prediction Gain (PG) is largest. 
 	The optimal parameters are found to be $N=1200$ and $O=1100$.
	\label{fig:HammingNOP10}
\end{minipage}% This must go next to `\end{minipage}`
\hspace{5mm}
\begin{minipage}[]{0.5\columnwidth}
	\begin{center}
	\input{HammingNOP10.tex}
	\end{center}
\end{minipage}

\vspace{10mm}
\begin{minipage}{.43\columnwidth}
	\textbf{System Delay Attenuation}\\
Without system delays, prediction is unnecessary, as the FXLMS algorithm (\textcolor{MATLABblue}{---}) alone, will achieve infinite attenuation. With increased system delay, an LP FXLMS (\textcolor{MATLABred}{---}), using the optimal parameters, has a higher attenuation. %of minimum 40 dB for delays below 14 samples at 48 $k$Hz.

%	If no system delay is present no prediction is needed, because the FXLMS algorithm (\textcolor{MATLABblue}{---}) achieves infinite attenuation.
%	The attenuation of the non-predicting algorithm decreases with delay. The predicting algorithm (\textcolor{MATLABred}{---}) however attenuate speech by $\geq$ 40 dB for delays of up to 14 samples at 48 $k$Hz.
\end{minipage}% This must go next to `\end{minipage}`
\hspace{5mm}
\begin{minipage}{0.5\columnwidth}
	\begin{center}
	\input{DelayRatio.tex}
	\end{center}
\end{minipage}
\vspace{10mm}

\begin{minipage}{.43\columnwidth}
\textbf{Frequency response comparison}\\
The frequency response of the FXLMS (\textcolor{MATLABblue}{---}) and LP FXLMS (\textcolor{MATLABred}{---}) are filtered using a 1/3 octave filter-bank.
The attenuation of the LP FXLMS is increased by up to 30 dB with $P=10$.

%The frequency content of the FXLMS, (\textcolor{MATLABblue}{---}) is not changed much by the predictor (\textcolor{MATLABred}{---}), but the attenuation is increased by $\geq$ 15 dB. 

%	The attenuation of different frequencies for the FXLMS (\textcolor{MATLABblue}{---}) and the LP FXLMS  systems. The frequency content is not changed much by the predictor, 
\end{minipage}% This must go next to `\end{minipage}`
\hspace{5mm}
\begin{minipage}{0.5\columnwidth}
	\begin{center}
	\input{SmoothCompareOur(fig).tex}
	\end{center}
\end{minipage}
